...
const PERSON_DESCRIPTION: Record<Locale, string> = {
  en: 'Senior software engineer with 8+ years building production systems across aviation, high-traffic consumer platforms, telecom, and retail. One of the principal engineers on a greenfield pilot-training platform at Pilatus Aircraft Ltd, deployed on-premise to enterprise customer environments. Daily AI-assisted workflow with Claude Code and custom tooling; shipped production LLM agents, RAG pipelines, and structured-output generation.',
  it: 'Senior software engineer con oltre 8 anni di esperienza nello sviluppo di sistemi in produzione nei settori aviazione, piattaforme consumer ad alto traffico, telecomunicazioni e retail. Tra gli ingegneri principali di una piattaforma greenfield di addestramento piloti presso Pilatus Aircraft Ltd, deployata on-premise presso ambienti enterprise dei clienti. Workflow quotidiano AI-assisted con Claude Code e tooling custom; ha rilasciato in produzione agenti LLM, pipeline RAG e generazione di output strutturati.',
}
...
export function getPersonSchema(locale: Locale): WithContext<Person> {
  return {
    ...
    hasOccupation: {
      ...
      skills:
        'TypeScript, React, .NET, Module Federation microfrontends, on-premise deployment, AI-assisted workflow, production LLM agents, RAG pipelines',
