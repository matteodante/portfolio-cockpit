/* ============================================================================
   ...
   the rest of the phone handling below is always on)
     The engine is phone-aware out of the box: on a coarse-pointer / ≤860px viewport it
       ...
       - keeps the still as a live poster until the clip actually paints its first frame,
         and primes each video (muted play→pause) the moment it is attached — this is what
         stops iOS from showing a blank scene before the first seek. The first pointer/touch
         re-primes every live clip, for the ones whose play() WebKit refused before the
         gesture.
   ...
   Each clip is downloaded IN FULL into a Blob and attached to the <video> as an object
   URL; the element is not even created until the last byte is in (see the long note
   above clipUrl). Scrubbing is random access, and a partially-buffered file answers a
   seek past its buffer by dropping to HAVE_METADATA and latching `seeking` — a frozen
   picture instead of a scrubbing video. The price is the resident bytes — blobs are
   kept once downloaded (a full scroll ends with the whole chain in memory; PREFETCH
   only paces the downloads, KEEP windows the decoders) — and a later reveal, with the
   still animating until then. Byte-range (206) support on your host is therefore NOT
   a requirement.
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  function attachVideo(s) {
    ...
    v.addEventListener('loadeddata', () => {
      try { v.pause(); } catch (e) {}
      // Kick a sub-frame seek so `seeked` fires even for a scene the visitor is
      // resting on (target 0 == currentTime 0 == no seek from the scrub loop).
      try { v.currentTime = 0.0001; } catch (e) {}
    });
    // Reveal (hide the still, show the video) ONLY on proof that a frame was
    // actually presented. rVFC is that proof; `seeked` is the fallback where rVFC
    // is unsupported. NEVER on loadeddata — on iOS/WebKit it fires while the
    // element is still paint-blank, which swaps the still for a black rectangle.
    ...
    // Prime IMMEDIATELY, not from a media event. Measured on real WebKit: a paused
    // blob video never decodes past HAVE_METADATA on its own — loadeddata never
    // fires, so any priming hung off it never runs, no frame is ever presented,
    // rVFC never certifies a paint and the scene stays a still forever. A muted
    // playsinline play() is gesture-exempt, presents the first frame, and is
    // paused right back. If it is refused (Low Power Mode), the reveal never
    // fires and the still stays — a stills page, never a black one.
    primeVideo(v);
  }
  ...
  // iOS needs a user gesture before a muted video will decode/paint reliably. Every
  // clip is already primed at attach time (see attachVideo), but a play() issued
  // before the first gesture can be refused outright; on the first touch we re-prime
  // every live clip so the first seek is instant instead of showing a blank frame.
  // `userReady` only makes that pass run once — nothing else reads it.
  let userReady = false;
  // Not just phones: desktop Safari is the same WebKit and needs the same nudge.
  // On Blink/Gecko the play resolves instantly and is paused right back — harmless.
...
function injectCSS() {
  ...
  /* The video is INVISIBLE until .has-clip certifies a genuinely painted frame.
     On iOS/WebKit a muted video that has never been primed paints nothing and the
     element composites BLACK — letting it sit above the still from the moment it
     attaches is a full black screen on every scene. The still and the video flip
     together, and only on proof of paint. */
  .sw-scene__still{will-change:transform;} .sw-scene.has-clip .sw-scene__still{opacity:0;}
  .sw-scene__video{z-index:1;opacity:0;} .sw-scene.has-clip .sw-scene__video{opacity:1;}
