...
function mountScrollWorld(container, config) {
  ...
  function attachVideo(s) {
    ...
    v.addEventListener('loadeddata', () => {
      try { v.pause(); } catch (e) {}
      // Prime unconditionally on touch devices: a muted playsinline play() is
      // allowed without a user gesture, and on iOS/WebKit it is the ONLY thing
      // that makes the decoder present a first frame. If it is refused anyway
      // (Low Power Mode), the reveal below never fires and the still simply
      // stays up — a stills-only page, never a black one.
      primeVideo(v);
