...
function mountScrollWorld(container, config) {
  ...
  SEGMENTS.forEach((s, i) => {
    ...
    s.loading = false; s.ready = false; s.failed = false;
  ...
  function loadClip(s) {
    ...
    const url = (usePortraitAssets() && s.clipM) ? s.clipM : s.clip;

    // A plain src, not a Blob: the decoder can start on the first buffered keyframe
    // and range-request the rest as we seek, instead of waiting for 3-6 MB to land
    // before anything paints. Requires the host to answer byte-range requests.
    ...
    // Reveal the video (hide the still poster) only once a real frame has actually
    // painted — rVFC is the only event that guarantees that. Note `seeked` alone is
    // not enough: a scene the visitor lands on at rest never seeks (target time 0 ==
    // currentTime 0), so its clip would stay hidden behind the still forever.
    ...
    else { v.addEventListener('seeked', reveal, { once: true }); v.addEventListener('loadeddata', reveal, { once: true }); }
    // One failure retires the scene to its still permanently. `failed` must be set
    // before unloadClip clears `loading`, or read() re-enters loadClip on the very
    // next frame and turns a single 404 into a per-rAF request storm.
    v.addEventListener('error', () => { s.failed = true; unloadClip(s); });
    v.src = url;
    s.el.appendChild(v); s.video = v; s.hasClip = true;
  }

  // Tear a clip back down to its still: detach the decoder and release the buffer.
  // Browsers cap concurrent hardware decoders — mobile Safari at a handful — so
  // keeping all nine alive is exactly what makes later scenes stop painting on a
  // phone. Setting src='' + load() is what actually frees the decoder; removing the
  // element alone does not.
  function unloadClip(s) {
    const v = s.video;
    s.video = null; s.hasClip = false; s.ready = false; s.loading = false; s.lastSeek = -1;
  ...
  // Keep a live clip only for the segments around the one on screen; everything
  // outside the window goes back to its still. Called from read(), so the window
  // follows the scroll.
  function syncClips(ci) {
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      const near = Math.abs(i - ci) <= KEEP;
      if (near) loadClip(s);
      else if (s.hasClip) unloadClip(s);
    }
  }
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // The incoming scene fades UP across the seam over the outgoing one. Holding
      // both at opacity 1 and only reordering z-index reads as a hard cut, not a
      // dissolve, because the top one is already fully opaque.
      // The first and last segments have no neighbour to dissolve with, so they must
      // not fade at their outer edge. The tail guard is load-bearing: max scrollY
      // exceeds the last segment's end by (maxVH - innerHeight) whenever the viewport
      // has ever been taller than it is now (a collapsed-then-restored mobile URL bar,
      // DevTools docked after load), which would otherwise fade the closing scene —
      // and the CTA on it — to flat background.
      const lead = (i === 0) ? 1 : clamp((y - s.start) / fade);
      const trail = (i === NSEG - 1) ? 1 : clamp((s.end + fade - y) / fade);
      const op = smooth(Math.min(lead, trail));
      s.el.style.opacity = op; s.visible = op > 0.001;
      // Hidden scenes must not stay composited: nine full-viewport layers is enough
      // GPU memory to stall a phone on its own.
  ...
  function destroy() {
    ...
    // Release every decoder and in-flight request before dropping the DOM. Emptying
    // the container alone leaves the media elements loading in the background, so a
    // remount (StrictMode double-mount, Fast Refresh, locale change) would stack a
    // second copy of the whole chain on the first.
